...
interface RightPanelProps {
  status: GenerateStatus; modulesV1: ModuleResult[]; modulesV2: ModuleResult[]
  versionLabelV1: string; versionLabelV2: string
...
function VersionCard({ version, label, modules, isGenerating, onAdopt, onAdoptAll }: { version: number; label: string; modules: ModuleResult[]; isGenerating: boolean; onAdopt: (key: string, content: string) => void; onAdoptAll: (modules: ModuleResult[]) => void }) {
...
export function RightPanel({ status, modulesV1, modulesV2, versionLabelV1, versionLabelV2, onAdopt, onAdoptAll }: RightPanelProps) {
  ...
  <div className="flex gap-6 min-w-fit">
    <div className="flex-1 min-w-0"><div className="flex items-center justify-between mb-3"><h3 className="text-sm font-semibold text-foreground">版本一{versionLabelV1 ? '：' + versionLabelV1 : ''}</h3>{modulesV1.every(m => m.status === 'completed') && (<button onClick={() => onAdoptAll(modulesV1)} className="inline-flex items-center rounded-md bg-[#07C160] px-3 py-1 text-xs font-medium text-white transition-all duration-150 hover:bg-[#06AD56] active:scale-95">采纳</button>)}</div></div>
    <div className="flex-1 min-w-0"><div className="flex items-center justify-between mb-3"><h3 className="text-sm font-semibold text-foreground">版本二{versionLabelV2 ? '：' + versionLabelV2 : ''}</h3>{modulesV2.every(m => m.status === 'completed') && (<button onClick={() => onAdoptAll(modulesV2)} className="inline-flex items-center rounded-md bg-[#07C160] px-3 py-1 text-xs font-medium text-white transition-all duration-150 hover:bg-[#06AD56] active:scale-95">采纳</button>)}</div></div>
  </div>
)}
{!isIdle && !isBlocked && (
        ...
        <div key={mod.moduleKey} className="flex gap-6">
          <div className="flex-1 min-w-0 rounded-lg border border-border bg-white overflow-hidden">
            <div className="flex items-center justify-between px-3 py-2 bg-muted/40 border-b border-border/40">
              ...
              {isCompleted && (<button onClick={() => onAdopt(mod.moduleKey, mod.content)} className="inline-flex items-center rounded-md border border-[#07C160] bg-transparent px-3 py-1 text-xs font-medium text-[#07C160] transition-all duration-150 hover:bg-[#07C160]/10 active:scale-95">采纳</button>)}
            </div>
            <div className="p-3">
              {isGenerating && mod.status === 'loading' && (<div className="flex flex-col gap-2"><Skeleton className="h-4 w-3/4" /><Skeleton className="h-4 w-full" /><Skeleton className="h-4 w-2/3" /></div>)}
          ...
          <div className="flex-1 min-w-0 rounded-lg border border-border bg-white overflow-hidden">
            <div className="flex items-center justify-between px-3 py-2 bg-muted/40 border-b border-border/40">
              ...
              {isCompleted2 && (<button onClick={() => onAdopt(mod2.moduleKey, mod2.content)} className="inline-flex items-center rounded-md border border-[#07C160] bg-transparent px-3 py-1 text-xs font-medium text-[#07C160] transition-all duration-150 hover:bg-[#07C160]/10 active:scale-95">采纳</button>)}
            </div>
            <div className="p-3">
              {isGenerating && mod2?.status === 'loading' && (<div className="flex flex-col gap-2"><Skeleton className="h-4 w-3/4" /><Skeleton className="h-4 w-full" /><Skeleton className="h-4 w-2/3" /></div>)}
              {isCompleted2 && (<div className="text-base text-foreground whitespace-pre-wrap" style={{ lineHeight: '1.7' }}>{mod2.content}</div>)}
            </div>
          </div>
        </div>
      )})}
